...
partition_table_t phys_partition_tables[MAX_PARTITIONS];
...
void post_init(void) {
    ...
    void *clb_buf = ps_dma_alloc(&ioops.dma_manager, clb_buf_size, 4, 1, PS_MEM_NORMAL);
    assert(clb_buf);
    ...
    void *ctba_buf = ps_dma_alloc(&ioops.dma_manager, ctba_buf_size, 4, 1, PS_MEM_NORMAL);
    assert(ctba_buf);
    ...
    void *fb_buf = ps_dma_alloc(&ioops.dma_manager, fb_buf_size, 4, 1, PS_MEM_NORMAL);
    assert(fb_buf);
    uintptr_t phys3 = ps_dma_pin(&ioops.dma_manager, fb_buf, fb_buf_size);
    assert(phys3 == (uintptr_t)fb_buf);

    int data_buf_size = BUF_SIZE;
    void *data_buf = ps_dma_alloc(&ioops.dma_manager, data_buf_size, 4, 1, PS_MEM_NORMAL);
    assert(data_buf);
    ...
    for (int client = 0; client < num_clients; client++) {
        ...
        ZF_LOGV("SATASERVER: Client %u Partitions: ", clients[client].client_id);
        ...
        /* Go through each partition assigned to the client and check to make sure the part isn't assigned */
    ...
    for (int client = 0; client < num_clients; client++) {
        ...
        for(int i=0; i<clients[client].num_partitions; i++)
        {
            ...
            ZF_LOGV("Client(%u): Virtual Partition %i:", clients[client].client_id, i);
            ZF_LOGV("\tStart: %u, Sectors: %u", clients[client].partition_tables[i].start_lba, clients[client].partition_tables[i].num_sectors);
            ZF_LOGV("\tBoot: %u, System ID: 0x%X", clients[client].partition_tables[i].boot, clients[client].partition_tables[i].sys_id);
        }
